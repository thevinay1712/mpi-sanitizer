...
int main(int argc, char** argv) {
    ...
    double max_diff = 1.0;
    int iter = 0;
    const int MAX_ITER = 10; // Run few iterations to trigger bug quickly
    ...
    while (max_diff > 1e-4 && iter < MAX_ITER) {
        ...
        // Reduce local max to Rank 0, then Broadcast back
        double global_max_diff = 0.0;
        
        // SEEDED COLLECTIVE MISMATCH BUG: Rank 0 skips the collective calls on iteration 5!
        if (rank == 0 && iter == 5) {
            max_diff = 0.1; // Bypass collective check on Rank 0
        } else {
            MPI_Reduce(&local_max_diff, &global_max_diff, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
            MPI_Bcast(&global_max_diff, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
