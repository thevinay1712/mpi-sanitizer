...
int main(int argc, char** argv) {
    ...
    while (max_diff > 1e-4 && iter < MAX_ITER) {
        ...
        if (rank < size - 1) {
            ...
            // SEEDED ALIASING BUG: Receiving directly into the sending buffer while Isend is active!
            MPI_Irecv(&grid[local_rows][0], N, MPI_DOUBLE, rank + 1, 1, MPI_COMM_WORLD, &reqs[req_count++]);
        }
    ...
    if (rank == 0) {
        std::cout << "Jacobi Solver completed." << std::endl;
    }
